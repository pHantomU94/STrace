...
def tranfer2dict(logline,starttime):
    row = {}
    parts = logline.split()
    row['src'] = 3 - int(parts[0][-1])      #source of this flow
    row['time'] = parts[1]                  #the time of this logline
    row['rtime'] = timestamp(parts[1]) - timestamp(starttime)   #the relative time of first flow 
    row['dst'] = int(parts[2].split('.')[-1].split(':')[0]) - 134   #destination of this flow
...
if __name__ == '__main__':
    path = 'Z:/spark_executor_logs_and_traces/ScalaPageRank/3/'
    #dirname = '/mnt/nas/spark_executor_logs_and_traces/ScalaPageRank/3/'
    dirname = path + 'app-20190402155632-0006/'
    appname = 'app-20190401203500-1549'
    tracename = 'trace.txt'
